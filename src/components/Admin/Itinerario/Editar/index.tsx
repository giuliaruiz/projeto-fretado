...
export default function EditarItinerario() {
    ...
    const [itinerarioSelecionado, setItinerarioSelecionado] = useState<Itinerario | null>(null)
    const [modalOpen, setModalOpen] = useState(false)
    const { itinerarios, setItinerarios, vanMap, motoristaMap, excluirItinerario } = useItinerario()

    const abrirModal = (itinerario: Itinerario) => {
                    ...
                    >
                        <div className="flex flex-col w-full justify-start items-start">
                            ...
                            <div className="grid grid-cols-2 w-full text-sm text-slate-300">
                                ...
                                <span className="flex justify-start">Motorista: {itinerario.motorista ? motoristaMap[itinerario.motorista] : "Nenhum motorista"}</span>
                                <span className="flex justify-start">Fim: {itinerario.final}</span>
                                <span className="flex justify-start">Van: {itinerario.van ? vanMap[itinerario.van] : "Nenhuma van"}</span>
                            </div>
                        </div>
